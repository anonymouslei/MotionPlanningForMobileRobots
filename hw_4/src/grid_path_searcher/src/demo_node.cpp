...
void visGoal(const Vector3d &target_point);
...
// suitable for time-variant system
Vector6d ode(const Vector6d &x, const Vector3d &u, double t) {
    Vector6d dx;
    dx << x(3), x(4), x(5), u(0), u(1), u(2);
    return dx;
}

Vector6d rk4(double t, const Vector6d &x, const Vector3d &u, double dt) {
    auto k1 = ode(x, u, t);
    auto k2 = ode(x + k1 * dt / 2, u, t + dt / 2);
    auto k3 = ode(x + k2 * dt / 2, u, t + dt / 2);
    auto k4 = ode(x + k3 * dt, u, t + dt);

    auto x_next = x + (k1 + 2 * k2 + 2 * k3 + k4) * dt / 6;
    return x_next;
}

Vector6d euler(double t, const Vector6d &x, const Vector3d &u, double dt) {
    auto dx = ode(x, u, t);
    return x + dx * dt;
}
    ...
    for (int i = 0; i <= _discretize_step; i++) { // acc_input_ax
        ...
        for (int j = 0; j <= _discretize_step; j++) { // acc_input_ay
            ...
            for (int k = 0; k <= _discretize_step; k++) { // acc_input_az
                ...
                acc_input(0) =
                    double(-_max_input_acc +
                           i * (2 * _max_input_acc / double(_discretize_step)));
                acc_input(1) =
                    double(-_max_input_acc +
                           j * (2 * _max_input_acc / double(_discretize_step)));
                acc_input(2) =
                ...
                for (int step = 0; step <= _time_step; step++) {
                    ...
                    STEP 1: finish the forward integration, the modelling has
                    been given in the document the parameter of forward
                    integration:
                    _max_input_acc|_discretize_step|_time_interval|_time_step
                    all have been given use the pos and vel to recored the steps
                    in the trajectory
                    ...
                    */
                    ...
                    auto x_next =
                        euler(step * delta_time, x, acc_input, delta_time);
                    pos = x_next.topRows(3);
                    vel = x_next.bottomRows(3);
                    ...
                    if (_homework_tool->isObsFree(coord_x, coord_y, coord_z) !=
                        1) {
                        collision = true;
                    }
                }
                /*
                   ...
                   Homeworktool::OptimalBVP the solving process has been given
                   in the document

                    because the final point of trajectory is the start point of
                   OBVP, so we input the pos,vel to the OBVP

                    after finish Homeworktool::OptimalBVP, the Trajctory_Cost
                   will record the optimal cost of this trajectory
                ...
                */
                ...
                // record the min_cost in the trajectory Library, and this is
                // the part pf selecting the best trajectory cloest to the
                // planning traget
...
int main(int argc, char **argv) {
    ...
    _grid_map_vis_pub =
        nh.advertise<sensor_msgs::PointCloud2>("grid_map_vis", 1);
    _path_vis_pub =
    ...
    for (int i = 0; i <= _discretize_step; i++) {
        for (int j = 0; j <= _discretize_step; j++) {
            for (int k = 0; k <= _discretize_step; k++) {
                ...
                Line.id = marker_id;
                for (int index = 0;
                     index < int(TraLibrary[i][j][k]->Position.size());
                     index++) {
...
void visGoal(const Vector3d &target_point) {
    ...
    ma.header.frame_id = "world";
    ma.header.stamp = ros::Time::now();
    ma.ns = "demo_node/Goal";
    ma.lifetime = ros::Duration();
    ma.action = visualization_msgs::Marker::ADD;
    ma.pose.orientation.w = 1.;
    ma.type = visualization_msgs::Marker::CUBE;
    ma.scale.x = _resolution / 5;
    ma.scale.y = _resolution / 5;
    ma.scale.z = _resolution / 5;
    ...
    ma.pose.position.x = target_point(0);
    ma.pose.position.y = target_point(1);
    ma.pose.position.z = target_point(2);

    _goal_vis_pub.publish(ma);
}
