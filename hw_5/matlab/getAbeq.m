function [Aeq beq]= getAbeq(n_seg, n_order, waypoints, ts, start_cond, end_cond)
    ...
    for i = 1: n_seg-1
        for j = 0:n_order-1
            Aeq_con_v(i, (j+1)+(i-1)*(n_order+1)) = (n_order-j)*ts(i)^(n_order-j-1);
        end
    end
    for i = 1: n_seg-1
    ...
    for i = 1: n_seg-1
        for j = 0:n_order-2
            Aeq_con_a(i, (j+1)+(i-1)*(n_order+1)) = (n_order-j)*(n_order-j-1)*ts(i)^(n_order-j-2);
        end
    end
    for i = 1: n_seg-1
        Aeq_con_a(i, (n_order-1)+i*(n_order+1)) = -2;
    end
    ...
    for i = 1: n_seg-1
        for j = 0:n_order-3
            Aeq_con_j(i, (j+1)+(i-1)*(n_order+1)) = (n_order-j)*(n_order-j-1)*(n_order-j-2)*ts(i)^(n_order-j-3);
        end
    end
    for i = 1: n_seg-1
        Aeq_con_j(i, (n_order-2)+i*(n_order+1)) = -6;
    end
